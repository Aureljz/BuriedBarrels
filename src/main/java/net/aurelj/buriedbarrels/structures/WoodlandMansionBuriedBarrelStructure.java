...
public class WoodlandMansionBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public WoodlandMansionBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!WoodlandMansionBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return WoodlandMansionBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "woodland_mansion_buried_barrel_start")),
                10
        );
