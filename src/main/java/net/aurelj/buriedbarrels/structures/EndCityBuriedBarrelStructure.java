...
public class EndCityBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public EndCityBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!EndCityBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return EndCityBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "end_city_buried_barrel_start")),
                10
        );
