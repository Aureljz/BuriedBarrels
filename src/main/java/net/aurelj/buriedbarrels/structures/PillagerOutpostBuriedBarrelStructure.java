...
public class PillagerOutpostBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public PillagerOutpostBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!PillagerOutpostBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return PillagerOutpostBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "pillager_outpost_buried_barrel_start")),
                10
        );
