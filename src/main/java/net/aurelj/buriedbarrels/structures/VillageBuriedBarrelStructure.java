...
public class VillageBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public VillageBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!VillageBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return VillageBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "village_buried_barrel_start")),
                10
        );
