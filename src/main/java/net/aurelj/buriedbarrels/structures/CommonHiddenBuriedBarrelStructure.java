...
public class CommonHiddenBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public CommonHiddenBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!CommonHiddenBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return CommonHiddenBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    private static boolean canGenerate(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        BlockPos spawnXZPosition = new BlockPos(context.chunkPos().getCenterX()-8, -1, context.chunkPos().getCenterZ()-8);
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        BlockPos blockpos = new BlockPos(context.chunkPos().getCenterX()-8, -1, context.chunkPos().getCenterZ()-8);
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "common_hidden_buried_barrel_start")),
                10
        );
