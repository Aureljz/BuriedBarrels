...
public class IglooBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public IglooBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!IglooBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return IglooBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "igloo_buried_barrel_start")),
                10
        );
