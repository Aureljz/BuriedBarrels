...
public class JungleTempleBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public JungleTempleBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!JungleTempleBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return JungleTempleBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "jungle_temple_buried_barrel_start")),
                10
        );
