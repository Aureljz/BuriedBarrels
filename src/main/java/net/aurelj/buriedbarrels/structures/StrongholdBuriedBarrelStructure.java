...
public class StrongholdBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public StrongholdBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!StrongholdBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return StrongholdBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "stronghold_buried_barrel_start")),
                10
        );
