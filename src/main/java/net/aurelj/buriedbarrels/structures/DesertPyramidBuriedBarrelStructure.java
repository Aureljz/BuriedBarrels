...
public class DesertPyramidBuriedBarrelStructure extends StructureFeature<StructurePoolFeatureConfig> {

    public DesertPyramidBuriedBarrelStructure(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, (context) -> {
            if (!DesertPyramidBuriedBarrelStructure.canGenerate(context)) {
                return Optional.empty();
            }
            else {
                return DesertPyramidBuriedBarrelStructure.createPiecesGenerator(context);
            }
        },
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(BuriedBarrelsMain.MODID, "desert_pyramid_buried_barrel_start")),
                10
        );
